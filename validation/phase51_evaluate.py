...
def main():
    ...
    result={
      ...
      "selected_export":{
        "feature_schema":"rich_v2","features":list(RICH_V2),
        "mode":rich_sel["mode"],"alpha":rich_sel["alpha"],
        "basis_names":basis_names(RICH_V2,rich_sel["mode"]),
        "mean":[float(x) for x in rich_model["mean"]],
        "scale":[float(x) for x in rich_model["scale"]],
        "theta":[float(x) for x in rich_model["theta"]],
        "solver_iterations":rich_model["iterations"],
      },
